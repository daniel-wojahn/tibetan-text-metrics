...
def test_compute_pairwise_analysis_pos(mock_model):
    # Prepare test data
    texts = {
        "file1": ["word1/n word2/v", "word3/adj"],
        "file2": ["word2/v word3/adj", "word1/n"]
    }
    file_names = ["file1", "file2"]

    # Run analysis
    result = compute_pairwise_analysis_pos(texts, file_names)
    ...
    assert "Normalized Syntactic Distance" in result.columns
    assert "Weighted Jaccard Similarity (%)" in result.columns
    assert "LCS Length" in result.columns
    assert "Normalized LCS (%)" in result.columns
    assert "Chapter Length 1" in result.columns
    assert "Chapter Length 2" in result.columns

    # Check data types
    assert result["LCS Length"].dtype in [np.int32, np.int64, np.float64]  # Allow float64 as it might be converted
